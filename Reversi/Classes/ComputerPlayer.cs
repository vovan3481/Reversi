...
namespace Reversi.Classes
{
    class ComputerPlayer : Player
    {
        ...
        private void DoNextMove()
        {
            ...
            solver.GetNextMove(out rowIndex, out columnIndex); //устанавливает значени фишки на поле из класса MoveSolver
            if (!this.Game.IsStopped && !this.Game.IsPaused && (rowIndex >= 0) && (columnIndex >= 0))
            {
                this.Game.Board.SetFieldColor(rowIndex, columnIndex, this.Color);//замена цвета поля(установка фишки)
            }
        }
